...
with open(os.path.join(BASE_DIR,"data/train_set_participants_demo_matched.txt")) as f:
    ids = f.readlines()
    train_ids = set([x.strip() for x in ids])

with open(os.path.join(BASE_DIR,"data/dev_set_participants_demo_matched.txt")) as f:
    ids = f.readlines()
    dev_ids = set([x.strip() for x in ids])

with open(os.path.join(BASE_DIR,"data/test_set_participants_demo_matched.txt")) as f:
    ids = f.readlines()
...
def evaluate_fusion_model(fusion_model, dataloader, prediction_models, config, split="dev", save_dev=False):
    fusion_model.eval()
    z_critical = scipy.stats.t.ppf(q=0.975, df = config["num_trials"]-1)
    ...
    if "test" in split:
        ...
        all_labels = all_labels[~uncertain_indices]
        all_final_predictions = all_final_predictions[~uncertain_indices]
        pd.DataFrame(uncertain_indices).to_csv('uncertain_indices_demo_matched.csv', index=False)
    ...
    if save_dev and (split=="dev"):
        with open(f"dev_results_seed_{config['seed']}.pkl", 'wb') as f:
            pickle.dump(
                {
                    'all_labels': all_labels,
                    'all_final_predictions': all_final_predictions,
...
'''
UFNet_withhold_predictions_demo_matched.py --batch_size=1024 --dropout_prob=0.2789655226950204 --gamma=0.8957000306409615 --hidden_dim=4 --learning_rate=0.25828673637410915 --minority_oversample=no --model_subset_choice=0 --momentum=0.8174184997051535 --num_buckets=100 --num_epochs=288 --num_trials=30 --optimizer=SGD --patience=5 --query_dim=128 --sampler=SVMSMOTE --scheduler=step --seed=404 --step_size=6 --uncertainty_weight=44.680661258116125 --use_scheduler=yes
'''

@click.command()
@click.option("--learning_rate", default=0.25828673637410915, help="Learning rate for classifier")
@click.option("--dropout_prob", default=0.2789655226950204)
@click.option("--num_buckets", default=100, help="Options: 5, 10, 20, 50, 100")
@click.option("--num_trials", default=30, help="Options: 100-1000")
@click.option("--uncertainty_weight", default=44.680661258116125)
@click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
@click.option("--sampler", default='SVMSMOTE', help="Options:SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
...
@click.option("--seed", default=404, help="Seed for random")
@click.option("--batch_size", default=1024)
@click.option("--num_epochs", default=288)
...
@click.option("--momentum", default=0.8174184997051535)
@click.option("--use_scheduler", default='yes', help="Options: yes, no")
@click.option("--scheduler", default='step', help="Options: step, reduce")
@click.option("--step_size", default=6)
@click.option("--gamma", default=0.8957000306409615)
...
def main(**cfg):
    ...
    df = df[df['id'].isin(all_ids)].reset_index(drop=True)
    print('Shape of the fusion dataframe:', df.shape)

    # save the whole fusion data
    df.to_csv(BASE_DIR + '/data/fusion_data_demo_matched.csv', index = False)

    fusion_ids = df[['id', 'label']]
    # save the fusion IDs
    fusion_ids.to_csv(BASE_DIR + '/data/fusion_ids_demo_matched.csv', index = False)
    ...
    test_df_to_save.to_csv(BASE_DIR + '/data/test_data_demo_matched.csv', index = False)
    ...
    print(f"Number of participants in the training set: {train_df['id'].nunique()}, Number of Positive participants: {train_df[train_df['label']==1.0]['id'].nunique()}")
    print(f"Number of participants in the validation set: {dev_df['id'].nunique()}, Number of Positive participants: {dev_df[dev_df['label']==1.0]['id'].nunique()}")
    print(f"Number of participants in the test set: {test_df['id'].nunique()}, Number of Positive participants: {test_df[test_df['label']==1.0]['id'].nunique()}")

    # Sanity check: there is no overlapping in participant IDs
    assert train_df['id'].nunique() + dev_df['id'].nunique() + test_df['id'].nunique() == df['id'].nunique(), "Overlapping participants found."
    ...
    test_metrics = evaluate_fusion_model(best_model, test_loader, prediction_models, cfg, split="test_data_demo_matched")
    if ENABLE_WANDB:
    ...
    # print("="*20)
    # print(evaluate_fusion_model(loaded_model, test_loader, prediction_models, cfg, split="test_data_demo_matched"))

if __name__ == "__main__":
    main()


#python UFNet_withhold_predictions_demo_matched.py --batch_size=1024 --dropout_prob=0.26926018177943645 --gamma=0.8671504381685116 --hidden_dim=8 --learning_rate=0.8447325796599376 --minority_oversample=no --model_subset_choice=0 --momentum=0.2982876770071527 --num_buckets=20 --num_epochs=228 --num_trials=30 --optimizer=SGD --patience=13 --query_dim=32 --sampler=SMOTENC --scheduler=step --seed=198 --step_size=14 --uncertainty_weight=96.7761329233072 --use_scheduler=yes 