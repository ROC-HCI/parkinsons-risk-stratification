...
def load(drop_correlated = True, corr_thr = 0.85, hand="both"):
    ...
    print(f"Number of unique ids in the demo matched dataset: {len(set(df['id']))}")
    ...
    df_features = df.drop(columns=['Unnamed: 0','filename','Protocol','Participant_ID','Task',
                'Duration','FPS','Frame_Height','Frame_Width','gender','age','race',
                'ethnicity','pd','dob','time_mdsupdrs','hand', 'id', 'date', 'id_date'])
...
'''
unimodal_finger_baal.py --batch_size=256 --beta1=0.8614051977519551 --beta2=0.838070344304056 --corr_thr=0.85 --drop_correlated=no --gamma=0.7469372002969898 --learning_rate=0.7188957650239818 --minority_oversample=yes --model=ShallowANN --momentum=0.8229517007015776 --num_epochs=85 --optimizer=SGD --patience=5 --random_state=4851 --scaling_method=StandardScaler --scheduler=step --seed=407 --step_size=48 --use_feature_scaling=yes --use_scheduler=no --weight_decay=0.09149240213547012
'''
...
@click.option("--learning_rate", default=0.7188957650239818, help="Learning rate for classifier")
@click.option("--random_state", default=4851, help="Random state for classifier")
@click.option("--seed", default=407, help="Seed for random")
...
@click.option("--num_epochs", default=85)
...
@click.option("--beta1", default=0.8614051977519551)
@click.option("--beta2", default=0.838070344304056)
@click.option("--weight_decay", default=0.09149240213547012)
@click.option("--momentum", default=0.8229517007015776)
@click.option("--use_scheduler", default='no', help="Options: yes, no")
@click.option("--scheduler", default='step', help="Options: step, reduce")
@click.option("--step_size", default=48)
@click.option("--gamma", default=0.7469372002969898)
