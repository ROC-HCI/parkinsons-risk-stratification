...
def load(drop_correlated = True, corr_thr = 0.85, feature_files=[WAVLM_FEATURES_FILE]):
    ...
    df = df[df['Participant_ID'].isin(all_ids)].reset_index(drop=True)
    print(f"Number of unique ids in the demo matched dataset: {len(set(df['Participant_ID']))}")
...
'''
unimodal_fox_baal.py --batch_size=128 --beta1=0.8910070603711491 --beta2=0.8874795542494529 --corr_thr=0.9 --drop_correlated=yes --gamma=0.5483748643185626 --learning_rate=0.17733935615464724 --minority_oversample=yes --model=ShallowANN --momentum=0.8687768218858828 --num_epochs=83 --optimizer=AdamW --patience=1 --random_state=139 --scaling_method=MinMaxScaler --scheduler=step --seed=965 --step_size=29 --use_feature_scaling=no --use_scheduler=no --weight_decay=0.01703385338538066
'''
...
@click.option("--learning_rate", default=0.17733935615464724, help="Learning rate for classifier")
@click.option("--random_state", default=139, help="Random state for classifier")
@click.option("--seed", default=965, help="Seed for random")
...
@click.option("--num_epochs", default=83)
...
@click.option("--beta1", default=0.8910070603711491)
@click.option("--beta2", default=0.8874795542494529)
@click.option("--weight_decay", default=0.01703385338538066)
@click.option("--momentum", default=0.8687768218858828)
@click.option("--use_scheduler", default='no', help="Options: yes, no")
@click.option("--scheduler", default='step', help="Options: step, reduce")
@click.option("--step_size", default=29)
@click.option("--gamma", default=0.5483748643185626)
