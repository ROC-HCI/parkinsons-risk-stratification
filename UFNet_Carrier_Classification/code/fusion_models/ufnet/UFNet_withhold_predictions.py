...
# with open(os.path.join(BASE_DIR,"data/train_set_participants.txt")) as f:
#     ids = f.readlines()
#     train_ids = set([x.strip() for x in ids])
    
with open(os.path.join(BASE_DIR,"data/dev_set_participants_full_cohort.txt")) as f:
    ids = f.readlines()
    dev_ids = set([x.strip() for x in ids])

with open(os.path.join(BASE_DIR,"data/test_set_participants_full_cohort.txt")) as f:
    ids = f.readlines()
...
def evaluate_fusion_model(fusion_model, dataloader, prediction_models, config, split="dev", save_dev=False):
    ...
    if "test" in split:
        ...
        all_labels = all_labels[~uncertain_indices]
        all_final_predictions = all_final_predictions[~uncertain_indices]
        pd.DataFrame(uncertain_indices).to_csv('uncertain_indices_full_cohort.csv', index=False)
...
'''
UFNet_withhold_predictions.py --batch_size=512 --dropout_prob=0.45622824444565746 --gamma=0.6964660013683557 --hidden_dim=128 --learning_rate=0.701188032741305 --minority_oversample=no --model_subset_choice=0 --momentum=0.7457242576660604 --num_buckets=20 --num_epochs=79 --num_trials=30 --optimizer=SGD --patience=12 --query_dim=32 --sampler=BorderlineSMOTE --scheduler=reduce --seed=561 --step_size=14 --uncertainty_weight=42.32262890809357 --use_scheduler=no

'''
@click.command()
@click.option("--learning_rate", default=0.701188032741305, help="Learning rate for classifier")
@click.option("--dropout_prob", default=0.45622824444565746)
@click.option("--num_buckets", default=20, help="Options: 5, 10, 20, 50, 100")
@click.option("--num_trials", default=30, help="Options: 100-1000")
@click.option("--uncertainty_weight", default=42.32262890809357)
@click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
@click.option("--sampler", default='BorderlineSMOTE', help="Options:SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
...
@click.option("--seed", default=561, help="Seed for random")
@click.option("--batch_size", default=512)
@click.option("--num_epochs", default=79)
...
@click.option("--momentum", default=0.7457242576660604)
@click.option("--use_scheduler", default='no', help="Options: yes, no")
@click.option("--scheduler", default='reduce', help="Options: step, reduce")
@click.option("--step_size", default=14)
@click.option("--gamma", default=0.6964660013683557)
@click.option("--patience", default=12)
...
def main(**cfg):
    ...
    for i in range(NUM_MODELS):
        ...
        # we print the model name and the config 
        print(f"Model Name:{model_name}")
        print(f"Model Config:{predictor_config}")
    ...
    df.to_csv(BASE_DIR + '/data/fusion_data_full_cohort.csv', index = False)

    fusion_ids = df[['id', 'label']]
    # save the fusion IDs
    fusion_ids.to_csv(BASE_DIR + '/data/fusion_ids_full_cohort.csv', index = False)
    ...
    test_df_to_save.to_csv(BASE_DIR + '/data/test_data_full_cohort.csv', index = False)
    ...
    test_metrics = evaluate_fusion_model(best_model, test_loader, prediction_models, cfg, split="test_data_full_cohort")
    if ENABLE_WANDB:
    ...
    # print("="*20)
    # print(evaluate_fusion_model(loaded_model, test_loader, prediction_models, cfg, split="test_save_full_cohort"))
